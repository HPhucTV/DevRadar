...
- V2 dùng direct PostgreSQL-backed orchestration theo ADR-006; không thêm Prefect nếu chưa có ADR mới với measured need. V3 hiện active nhưng phải khóa evaluation dataset/baseline trước; LLM provider và pgvector vẫn `Proposed` tới khi spike/task tương ứng đạt. LangGraph chỉ từ V4; Next.js từ V5.
