...
- V2 dùng direct PostgreSQL-backed orchestration theo ADR-006; không thêm Prefect nếu chưa có ADR mới với measured need. V3 evaluation baseline đã khóa; DeepSeek generation và embedding/pgvector boundary trong ADR-008 vẫn `Proposed` cho tới khi live credential/latency/evaluation gate đạt. Không thêm SDK, model call production hoặc pgvector migration chỉ dựa trên candidate spike. LangGraph chỉ từ V4; Next.js từ V5.
