...
- V2 dùng direct PostgreSQL-backed orchestration theo ADR-006; không thêm Prefect nếu chưa có ADR mới với measured need. V3 evaluation baseline đã khóa; ADR-008 chỉ chấp nhận DeepSeek cho synthetic generation boundary, còn ADR-009 chấp nhận fixed-revision local multilingual E5 + exact pgvector cho private V3. Không thêm external embedding provider, HNSW, model call production hoặc provider SDK khác nếu chưa có evaluation/privacy/latency gate và ADR mới. LangGraph chỉ từ V4; Next.js từ V5.
