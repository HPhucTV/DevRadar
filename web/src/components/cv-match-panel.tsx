    ...
    <>
      <section className="cv-layout" aria-label="Local resume matching">
        <form className="content-section cv-form cv-upload-card" onSubmit={submitUpload} ref={formRef}>
          ...
          <label className="upload-dropzone">Resume file<input type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><small className="field-help">PDF or DOCX, maximum 5 MiB. Raw text and the original file are not shown here.</small></label>
          <button className="button-primary" type="submit" disabled={busy}>{busy ? "Processing locally..." : "Upload and find matches"}</button>
        </form>
        {profile ? <section className="content-section profile-summary profile-card"><div className="section-heading"><div><p className="eyebrow">Ephemeral profile</p><h2>{profile.fileName}</h2></div><span>Expires {date(profile.expiresAt)}</span></div><dl><dt>Status</dt><dd>{profile.extractionStatus} · {profile.sourceFormat}</dd><dt>Skills</dt><dd>{profile.skills.join(", ") || "No accepted skills"}</dd><dt>Roles</dt><dd>{profile.roles.join(", ") || "No accepted roles"}</dd><dt>Locations</dt><dd>{profile.locations.join(", ") || "No location evidence"}</dd></dl><button className="danger-button" type="button" onClick={removeProfile} disabled={busy}>Delete profile and matches</button></section> : <section className="content-section api-state privacy-note"><strong>Private by default</strong><p>The profile lives for the protected TTL only. Delete it when you are done.</p></section>}
      </section>
      {error ? <ApiErrorState error={error} /> : null}
      {notice ? <p className="status-message" role="status">{notice}</p> : null}
      {generation ? <section className="content-section match-results"><div className="section-heading"><div><p className="eyebrow">Versioned ranking</p><h2>{generation.storedMatches} current matches</h2></div><button type="button" onClick={refreshMatches} disabled={busy}>Refresh matches</button></div><div className="metric-grid"><div className="metric"><span>Scoring</span><strong>{generation.scoringVersion}</strong><small>{generation.createdMatches} new · {generation.reusedMatches} reused</small></div><div className="metric"><span>Available jobs</span><strong>{generation.availableJobs}</strong><small>{generation.unavailableJobs} without compatible vectors</small></div><div className="metric"><span>Considered</span><strong>{generation.consideredJobs}</strong><small>Generated {date(generation.generatedAt)}</small></div></div>{matches.length ? <div className="match-list">{matches.map((match) => <article className="match-card" key={match.id}><div className="section-heading"><div><p className="eyebrow">{match.job.companyName}</p><h3>{match.job.title}</h3><span>{match.job.location || "Location unavailable"}</span></div><strong className="match-score score-tile">{percent(match.overallScore)}</strong></div><p className="match-meta evidence-meta">Evidence {percent(match.evidenceCoverage)} · {match.job.status} · {match.scoringVersion}</p><div className="tag-columns skill-columns"><div className="matched-skills"><strong>Matched</strong><p>{match.matchedSkills.join(", ") || "No skill match evidence"}</p></div><div className="missing-skills"><strong>Missing</strong><p>{match.missingSkills.join(", ") || "No missing skill evidence"}</p></div></div><ul className="explanation-list">{match.explanation.map((item) => <li key={item}>{item}</li>)}</ul><a href={match.job.sourceUrl} target="_blank" rel="noreferrer">Open source posting</a></article>)}</div> : <div className="api-state empty-state"><strong>No current matches</strong><p>No active compatible job vectors produced a match for this profile.</p></div>}</section> : null}
    </>
  );
}
