...
export default async function OverviewPage() {
  ...
  return <>
    <section className="route-header route-header--compact"><p className="route-label">{dictionary.overview.eyebrow}</p><h1>{dictionary.overview.title}</h1><p>{dictionary.overview.body}</p></section>
    <section aria-label={dictionary.overview.title} className="metrics-layout">
    ...
    <div className="overview-layout">
      <section className="glass-surface data-surface comparison-panel">
        <div className="section-heading"><div><p className="route-label">{dictionary.overview.demandEyebrow}</p><h2>{dictionary.overview.demandTitle}</h2></div><span>{skills.kind === "success" ? `${formatNumber(skills.value.meta.analyzedJobs, locale)} ${dictionary.common.analyzed}` : dictionary.common.unavailable}</span></div>
        {skills.kind === "success" && topSkills.length ? <div className="comparison-list">{topSkills.map((skill) => <div className="comparison-row" key={skill.name}><div className="skill-demand-label"><strong>{skill.name}</strong><span>{formatNumber(skill.jobCount, locale)} {dictionary.common.jobs} · {formatPercent(skill.share, locale)}</span></div><div aria-hidden="true" className="skill-demand-track"><span style={{ width: `${Math.max(6, Math.min(100, skill.share * 100))}%` }} /></div></div>)}</div> : skills.kind === "success" ? <EmptyState message={dictionary.overview.noSkills} /> : <ApiErrorState error={skills} />}
      </section>
      <section className="glass-surface data-surface activity-panel"><div className="section-heading"><div><p className="route-label">{dictionary.overview.latestEyebrow}</p><h2>{dictionary.overview.latestTitle}</h2></div><Link href="/jobs">{dictionary.overview.explore}</Link></div>{jobs.kind === "success" ? jobs.value.data.length ? <JobList compact jobs={jobs.value.data} /> : <EmptyState message={dictionary.overview.noJobs} /> : <ApiErrorState error={jobs} />}</section>
    </div>
  </>;
}
