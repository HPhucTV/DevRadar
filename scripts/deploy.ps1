...
try {
    ...
    if (-not $SkipBuild) {
        ...
        & docker build --pull --tag $WebImage web
        ...
        if ($LASTEXITCODE -ne 0) { throw "Requested web image '$WebImage' is not available locally." }
    }
    ...
    if ($LASTEXITCODE -ne 0) { throw "API/web services did not become healthy." }
    ...
    Write-Output "deploy=pass api_image=$Image web_image=$WebImage project=$ProjectName"
}
finally {
