...
Tài liệu này định nghĩa bằng chứng cần có để nói DevRadar chạy đúng, an toàn và có thể chẩn đoán. Scaffold, PostgreSQL schema/migration, safe HTTP/raw snapshot boundary, ba V1 source adapters và current-state Job upsert đã có verified test/static/live evidence; run-level orchestration và domain API gates vẫn chưa pass.
