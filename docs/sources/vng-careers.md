...
- Discover từ `/tim-kiem-viec-lam?page=<n>`.
- Parse JSON trong `<script id="__NEXT_DATA__" type="application/json">`; không đoán hoặc gọi private endpoint từ JavaScript bundle.
- ListingRef tối thiểu gồm `job_id`, `slug`, canonical detail URL, title, location và job family raw.
...
5. mọi item trong approved IT family có detail URL hợp lệ; item ngoài scope bị filter có counter riêng, không bị coi là parse failure.
...
Live smoke tối đa hai list page và một detail, dùng User-Agent/rate policy ở trên; không chạy schedule trước khi fixture tests pass.
