...
| [ADR-011](0011-accept-secondary-remote-api-cohort.md) | Superseded in current runtime | Cohort remote thứ cấp lịch sử; static adapter bị ADR-026 thay thế |
...
| [ADR-026](0026-accept-owner-overridden-source-recipes.md) | Superseded in part | SourceRecipe owner-local/no-code vẫn hiệu lực; terms acknowledgement bị ADR-029 loại |
...
| [ADR-029](0029-remove-source-terms-acknowledgement-retain-technical-barriers.md) | Accepted | Xóa source terms/acknowledgement; giữ technical barriers và dẫn completion tới Jobs explorer |

## Candidate technology chưa được chấp nhận

Prefect đã được đánh giá và defer theo ADR-006; chỉ đánh giá lại khi có measured need. ADR-007 được giữ để truy vết nhưng đã `Superseded` bởi ADR-008. ADR-010 hiện hành chấp nhận local multilingual MiniLM + exact pgvector cho V3 private deployment. ADR-012 vẫn hoãn LangGraph; ADR-013 loại direct V4 agent runtime vì không có measurable usefulness gain, nên hiện không có agent runtime được chấp nhận. ADR-026 tiếp tục khóa owner-local/no-code SourceRecipe và technical no-bypass boundary; ADR-029 loại terms notice/acknowledgement khỏi active runtime. ADR-004/ADR-011/ADR-024 và source-specific evidence chỉ còn giá trị lịch sử. ADR-015 khóa auth strategy cho V6-002 nhưng chưa phải implementation evidence. ADR-016 khóa CI/deploy/rollback boundary; ADR-019 đã thay scanner container bằng Trivy pinned, ADR-020 mở rộng release/scan sang standalone web artifact và ADR-022 thay official Caddy artifact trong ADR-021 bằng patched scratch image. ADR-023 khóa backup/Uptime contract nhưng chưa cấp provider evidence; account, bucket, host, domain và credential vẫn là prerequisite do operator cung cấp. Public deployment/restore vẫn cần phase gate riêng. External embedding provider, HNSW và mọi công nghệ V5–V6 vẫn cần phase gate/evidence riêng. Quyết định khó đảo ngược cần ADR mới hoặc amendment qua ADR kế tiếp, không âm thầm sửa lịch sử.
