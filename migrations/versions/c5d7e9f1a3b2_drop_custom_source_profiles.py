"""Drop the purged legacy custom source profile table.
...
def upgrade() -> None:
    remaining = op.get_bind().scalar(sa.text("SELECT count(*) FROM custom_source_profiles"))
    if remaining:
        raise RuntimeError("custom_source_profiles must be empty before the hard cut")
...
def downgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "length(btrim(name)) > 0",
            name="ck_custom_source_profiles_name_not_blank",
        ),
        sa.CheckConstraint(
            "base_url ~ '^[!-~]+$' "
